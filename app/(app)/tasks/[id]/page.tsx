...
export default function TaskDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [documents, setDocuments] = useState<DbRecord[]>([])
  ...
  async function loadRelatedOptions() {
    const [c, co, d] = await Promise.all([fetchRecords("contact"), fetchRecords("company"), fetchRecords("document")])
    setContacts(c)
    setCompanies(co)
    setDocuments(d)
  }
  ...
  const displayTitle = record.title || (record.properties.title as string) || "Task"
    ...
    >
      <Grid cols={2} gap="lg">
        <Stack gap="md">
          <Panel variant="bordered">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-foreground">{editing ? "Edit Task" : "Details"}</h3>
              {editing && (
                <Button
                  variant="ghost"
                  size="icon"
        ...
        <Stack gap="md">
          ...
          <Panel variant="bordered">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  setAddingRelation(addingRelation === "task_relates_to_contact" ? null : "task_relates_to_contact")
                }
              >
            ...
            {addingRelation === "task_relates_to_contact" && (
              <div className="flex items-center gap-2 mb-2">
                ...
                <Button size="sm" onClick={() => handleAddRelation("task_relates_to_contact")}>
                  Add
                </Button>
            ...
            {relationsFrom.filter((r) => r.relationship_key === "task_relates_to_contact").length === 0 &&
              addingRelation !== "task_relates_to_contact" && (
          ...
          <Panel variant="bordered">
            <div className="flex items-center justify-between mb-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  setAddingRelation(addingRelation === "task_relates_to_company" ? null : "task_relates_to_company")
                }
              >
                <Plus className="w-3 h-3" />
                Add
            ...
            {addingRelation === "task_relates_to_company" && (
              <div className="flex items-center gap-2 mb-2">
                <Select value={selectedRelationTarget} onValueChange={setSelectedRelationTarget}>
                  <SelectTrigger className="flex-1">
                ...
                <Button size="sm" onClick={() => handleAddRelation("task_relates_to_company")}>
                  Add
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setAddingRelation(null)}>
                  <X className="w-3 h-3" />
            ...
            {relationsFrom.filter((r) => r.relationship_key === "task_relates_to_company").length === 0 &&
              addingRelation !== "task_relates_to_company" && (
          ...
          <Panel variant="bordered">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-muted-foreground">Linked Documents</span>
              <Button
                variant="ghost"
                size="sm"
                onClick={() =>
                  setAddingRelation(addingRelation === "task_links_to_document" ? null : "task_links_to_document")
                }
              >
                <Plus className="w-3 h-3" />
                Add
            ...
            {addingRelation === "task_links_to_document" && (
              <div className="flex items-center gap-2 mb-2">
                <Select value={selectedRelationTarget} onValueChange={setSelectedRelationTarget}>
                  <SelectTrigger className="flex-1">
                ...
                <Button size="sm" onClick={() => handleAddRelation("task_links_to_document")}>
                  Add
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setAddingRelation(null)}>
                  <X className="w-3 h-3" />
            ...
            {relationsFrom
              .filter((r) => r.relationship_key === "task_links_to_document")
              .map((rel) => {
                const related = relatedRecords[rel.to_record_id]
                return (
                  <div
                  ...
                  >
                    <span
                      className="text-sm text-foreground cursor-pointer hover:underline"
                      onClick={() => router.push(`/documents/${rel.to_record_id}`)}
                    >
            ...
            {relationsFrom.filter((r) => r.relationship_key === "task_links_to_document").length === 0 &&
              addingRelation !== "task_links_to_document" && (
